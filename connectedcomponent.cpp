...
is ** IITkgp_functions::labelingdfs(is **tempstructuredimage,int *m, int *n, int temp_row, int temp_col)
{
  ...
  int i,j,l,k;
    ...
    for(l=i-1;l<=i+1;l++)
    {
      for(k=j-1;k<=j+1;k++)
      {
        ...
        int temp_val = validate(l,k,temp_row,temp_col);
        if( temp_val == 1)
        {
...
is** IITkgp_functions::cclabeling(is **tempstructuredimage,int temp_row, int temp_col)
{
    int label=1;
    int i,j;
...
nocc* IITkgp_functions::FindConnectedComponent(Mat image, Mat src_gray)
{
    int row,col;
    int i,j,*mapping,k;
    ncc=0;
